...
export async function getBasicGames(limit?: number, offset?: number): Promise<BasicGame[]> {
...
export async function deleteGame(id: number): Promise<boolean> {
  ...
  await db.delete(usersToGames)
    .where(eq(usersToGames.gameId, id));
...
export async function getGamesByPlatform(platformId: number, limit?: number, offset?: number): Promise<Game[]> {
  let query = db.select()
    .from(games)
...
// Get games by genre
export async function getGamesByGenre(genreId: number, limit?: number, offset?: number): Promise<Game[]> {
  let query = db.select()
    .from(games)
