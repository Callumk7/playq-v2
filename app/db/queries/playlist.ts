...
export async function createPlaylist(
	...
	if (gameIds && gameIds.length > 0) {
		await db.insert(gamesToPlaylists).values(
			gameIds.map((gameId) => ({
				gameId,
...
export async function getPlaylistById(id: string): Promise<PlaylistWithGames | null> {
	...
	const gameConnections = await db
		.select()
...
export async function getPlaylistsWithGames(
	creatorId?: string,
	includePrivate = false,
): Promise<PlaylistWithGames[]> {
	...
	const gameIdsByPlaylist = gameConnections.reduce(
		(acc, gc) => {
...
export async function updatePlaylist(
	...
	if (gameIds !== undefined) {
		...
		await db.delete(gamesToPlaylists).where(eq(gamesToPlaylists.playlistId, id));
...
export async function removeGamesFromPlaylist(
	playlistId: string,
	gameIds: number[],
): Promise<boolean> {
	if (gameIds.length === 0) {
