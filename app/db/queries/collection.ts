...
export type GameWithDetails = typeof games.$inferSelect;

// Add a game to user's collection
export async function addGameToCollection(userId: string, gameId: number): Promise<boolean> {
  try {
...
export async function removeGameFromCollection(userId: string, gameId: number): Promise<boolean> {
  try {
    const result = await db.delete(usersToGames)
      .where(and(
        eq(usersToGames.userId, userId),
        eq(usersToGames.gameId, gameId)
      ));
...
export async function isGameInCollection(userId: string, gameId: number): Promise<boolean> {
...
export async function getUserGameIds(userId: string): Promise<number[]> {
  const relations = await db.select({ gameId: usersToGames.gameId })
    .from(usersToGames)
...
export async function getUserGames(userId: string): Promise<GameWithDetails[]> {
...
export async function addGamesToCollection(userId: string, gameIds: number[]): Promise<boolean> {
  if (gameIds.length === 0) {
    return true;
  }
...
export async function removeGamesFromCollection(userId: string, gameIds: number[]): Promise<boolean> {
  if (gameIds.length === 0) {
    return true;
  }
  
  try {
    await db.delete(usersToGames)
      .where(and(
        eq(usersToGames.userId, userId),
        inArray(usersToGames.gameId, gameIds)
      ));
...
export async function getUsersWithGame(gameId: number): Promise<string[]> {
  const relations = await db.select({ userId: usersToGames.userId })
    .from(usersToGames)
